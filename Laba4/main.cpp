...
cv::Mat good_picture(cv::Mat input_img, cv::Mat *planes_4_func, bool isForward){
    ...
    cv::magnitude(planes_4_func[0], planes_4_func[1], planes_4_func[0]);// planes[0] = magnitude
    cv::Mat magI = planes_4_func[0];
...
void backward_fourier_tf_comparison(cv::Mat starting_image, std::array<cv::Mat, 2> transformed_img){
    ...
    cv::Mat res_my_back  = good_picture(tf_img_my_back, planes_my_back, false);
    cv::Mat res_builtIn_back = good_picture(tf_img_builtIn_back, planes_builtIn_back, false);
...
void correlation_task(cv::Mat first_image, cv::Mat second_image, std::string win_name){
    ...
    cv::Mat padded_backwar;
    cv::copyMakeBorder(first_image, padded_backwar, 0, second_image.rows, 0,
                       second_image.cols, cv::BORDER_CONSTANT, cv::Scalar::all(0));
    cv::Mat planes_backward[] = {cv::Mat_<float>(padded_backwar), cv::Mat::zeros(padded_backwar.size(), CV_32F)};
    cv::Mat complexImg_backward;
    cv::merge(planes_backward, 2, complexImg_backward);
    ...
    cv::Mat res_multed  = good_picture(multed_img_normal, planes_backward, true);
    ...
    cv::Mat out_picture;
    cv::threshold(ready_picture, out_picture, threshold, 1000, cv::THRESH_BINARY);
...
int main()
{
    cv::Mat starting_image = cv::imread("C:/WIP/Tex3penie/Laba_4_Clion/Fourier.jpeg", cv::IMREAD_GRAYSCALE);
    cv::resize(starting_image, starting_image, cv::Size(512, 512));


    cv::Mat number_image = cv::imread("C:/WIP/Tex3penie/Laba_4_Clion/Number.jpeg", cv::IMREAD_GRAYSCALE);
    ...
    cv::Mat A_image = cv::imread("C:/WIP/Tex3penie/Laba_4_Clion/A.jpeg", cv::IMREAD_GRAYSCALE);
    cv::bitwise_not(A_image, A_image);
